...
6. For a project big enough that "what is this" needs answering: describe the architecture in a few lines — main components, how they fit, the one or two design decisions that shape it — and a short tree of the key directories with one line of role each. Link a diagram rather than re-prosing it (see the `diagram` skill).
7. Add short sections as warranted: Requirements/prerequisites, Development/contributing, License.
8. Run the install and usage commands yourself; fix the README to match reality before finishing.

## Rules
- Lead with what it does and why, not a logo or badge wall; a reader should have "what & why" in the first three lines.
- Accurate over impressive — describe what the code actually does, and be honest about what is done vs planned.
