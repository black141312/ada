...
<!-- Vendored from github.com/ningzimu/codex-ppt-skill @ 7e5437f (MIT, see LICENSE.upstream).
     Local edits: this "Running inside ada" section, plus `category:` above. -->

## Running inside ada

ada's runtime differs from the one upstream assumes. These overrides win over anything below:

- **No subagents.** ada has no subagent tool, so the mandatory per-slide dispatch in step 9 and
  "Subagent Dispatch" cannot happen. Generate the slides **sequentially** in the main loop — that is
  the sanctioned path here, not a degraded fallback, and is _not_ a blocker. Everything else in the
  slide-job contract still applies: prepare jobs, record every dispatch and result with the bundled
  scripts, and keep `slide_jobs.json` / `slide_run_state.json` truthful.
- **The built-in image backend is `generate_image`** — it writes a PNG straight to a path you choose,
  which is what `origin_image/slide_XX.png` needs. Sizes are `1024x1024`, `1536x1024` (use this one
  for slides — nearest to 16:9) and `1024x1536`. Use `scripts/image_gen.py` only if that tool fails.
- **Assembly needs Python.** `scripts/assemble_ppt.py` imports python-pptx and Pillow. Check with
  `python -c "import pptx, PIL"` before promising a `.pptx`; if it fails, run
  `pip install -r <skill dir>/requirements.txt` (ask first) or report it as a blocker up front rather
  than after generating a dozen images.
- Relative paths in this file resolve against the skill directory printed at the end of `use_skill`,
  not the project — prefix them.
