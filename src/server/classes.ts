...
type ProgressRow = { progress: string; updated_at: number | string };
...
export async function listClasses(user: string): Promise<ClassSummary[]> {
  ...
  const opened = await all<Row & { p_updated_at: number | string }>(
    ...
    `select c.*, p.updated_at as p_updated_at from class_progress p join classes c on c.id = p.class_id
     where p.user_id = $1 and c.owner <> $2 and c.share_token is not null order by c.updated_at desc`,
    [user, user],
  );
...
export async function getShared(user: string, token: string): Promise<{ id: string; doc: unknown; progress: unknown | null } | null> {
  ...
  if (!r) return null;
  // Remembering "this user opened it" is what puts the class in their list from now on.
  await run(
    `insert into class_progress (class_id, user_id, progress, updated_at) values ($1,$2,'{}',0)
...
export async function putProgress(user: string, id: string, progress: unknown): Promise<{ ok: true; stale: boolean } | { ok: false; status: 403 }> {
  ...
  const cur = await one<ProgressRow>("select progress, updated_at from class_progress where class_id = $1 and user_id = $2", [id, user]);
  const allowed = r.owner === user || (!!cur && !!r.share_token);
