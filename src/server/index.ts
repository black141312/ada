...
// Device sign-in page: Continue with GitHub / Google → (return signed in) → auto-approve the code.
// Buttons are rendered from whichever social providers actually have creds in env (the same check
// auth.ts makes), so an unconfigured provider never shows a button that 500s on click.
